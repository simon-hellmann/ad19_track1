%% validate_ADM1_R3_x2.m
% Cross-checks ADM1-R3-x2 against reference ADM1-R3 by evaluating both ODE
% and output functions at identical (x0, u, xi) conditions. Since x2 merely
...
% g vectors when theta_x2 carries the same nominal values.
%
...
[c_x2, ~, odeFunc_x2, measFunc_x2, theta0_x2, ~, ~, ~] = ...
...
[f_x2, I_ac_x2, I_ph_x2, I_Nlim_x2, I_nh3_x2] = odeFunc_x2(x0, u_test, xi_test, theta0_x2);
...
fprintf("\n=== ODE: f_x2 vs f_r3 ===\n");
fprintf("%-10s  %13s  %13s  %13s\n", "State", "f_r3", "f_x2", "|diff|");
for k = 1:14
    fprintf("%-10s  %13.6e  %13.6e  %13.6e\n", ...
        state_names{k}, f_r3(k), f_x2(k), abs(diff_f(k)));
end
...
fprintf("%-8s  %10.6f  %10.6f  %10.2e\n", "I_ph",   I_ph_r3,   I_ph_x2,   abs(I_ph_x2   - I_ph_r3));
fprintf("%-8s  %10.6f  %10.6f  %10.2e\n", "I_Nlim", I_Nlim_r3, I_Nlim_x2, abs(I_Nlim_x2 - I_Nlim_r3));
fprintf("%-8s  %10.6f  %10.6f  %10.2e\n", "I_nh3",  I_nh3_r3,  I_nh3_x2,  abs(I_nh3_x2  - I_nh3_r3));
fprintf("%-8s  %10.6f  %10.6f  %10.2e\n", "I_ac",   I_ac_r3,   I_ac_x2,   abs(I_ac_x2   - I_ac_r3));
